...
pub fn build_system_sets(app: &mut App) {
    ...
    // This is how iyes_loopless implements a system condition on a state.
    app.add_system(player_input::player_input.run_in_state(AwaitingInput));

    app.add_system_set(
        // This is a SystemSet, conditional to a state, as implemented by iyes_loopless.
        ConditionSet::new()
            .run_in_state(PlayerCollisions)
    ...
    app.add_system_set(
        ConditionSet::new()
            .run_in_state(MonsterCollisions)
            .with_system(collisions::collisions)
            .with_system(next_step::next_step)
            .into(),
    );
