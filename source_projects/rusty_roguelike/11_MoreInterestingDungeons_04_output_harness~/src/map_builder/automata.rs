...
impl CellularAutomataArchitect {
    ...
    fn count_neighbors(&self, x: i32, y: i32) -> usize {
        ...
        if self.map.tiles[map_idx(x - 1, y)] == TileType::Wall {
            neighbors += 1;
        }
        if self.map.tiles[map_idx(x + 1, y)] == TileType::Wall {
            neighbors += 1;
        }
        if self.map.tiles[map_idx(x, y - 1)] == TileType::Wall {
            neighbors += 1;
        }
        if self.map.tiles[map_idx(x, y + 1)] == TileType::Wall {
            neighbors += 1;
        }
        if self.map.tiles[map_idx(x - 1, y - 1)] == TileType::Wall {
            neighbors += 1;
        }
        if self.map.tiles[map_idx(x + 1, y - 1)] == TileType::Wall {
            neighbors += 1;
